...
class Html
{
    ...
    /** @var callable[] */
    protected array $start_tag_callbacks;
    /** @var callable[] */
    protected array $end_tag_callbacks;
    /** @var mixed[] */
    private array $stack = [];
    public string $string_data = '';
    private Rich_Text $rich_text_object;
    private bool $preserve_white_space = false;
    public function __construct()
    {
        if (!isset($this->start_tag_callbacks)) {
            $this->start_tag_callbacks = ['font' => $this->start_font_tag(...), 'b' => $this->start_bold_tag(...), 'strong' => $this->start_bold_tag(...), 'i' => $this->start_italic_tag(...), 'em' => $this->start_italic_tag(...), 'u' => $this->start_underline_tag(...), 'ins' => $this->start_underline_tag(...), 'del' => $this->start_strikethru_tag(...), 's' => $this->start_strikethru_tag(...), 'sup' => $this->start_superscript_tag(...), 'sub' => $this->start_subscript_tag(...)];
        }
        if (!isset($this->end_tag_callbacks)) {
            $this->end_tag_callbacks = ['font' => $this->end_font_tag(...), 'b' => $this->end_bold_tag(...), 'strong' => $this->end_bold_tag(...), 'i' => $this->end_italic_tag(...), 'em' => $this->end_italic_tag(...), 'u' => $this->end_underline_tag(...), 'ins' => $this->end_underline_tag(...), 'del' => $this->end_strikethru_tag(...), 's' => $this->end_strikethru_tag(...), 'sup' => $this->end_superscript_tag(...), 'sub' => $this->end_subscript_tag(...), 'br' => $this->break_tag(...), 'p' => $this->break_tag(...), 'h1' => $this->break_tag(...), 'h2' => $this->break_tag(...), 'h3' => $this->break_tag(...), 'h4' => $this->break_tag(...), 'h5' => $this->break_tag(...), 'h6' => $this->break_tag(...)];
        }
    }
     ...
     */
    public function to_rich_text_object(string $html, bool $preserve_white_space = false): Rich_Text
    {
        $this->initialise();
        ...
        @$dom->load_html($prefix . $html, LIBXML_HTML_NOIMPLIED | LIBXML_HTML_NODEFDTD);
        //    Discard excess white space
        $dom->preserve_white_space = false;
        $this->rich_text_object = new Rich_Text();
        $this->preserve_white_space = $preserve_white_space;
        $this->parse_elements($dom);
        $this->preserve_white_space = false;
        // Clean any further spurious whitespace
        $this->clean_whitespace();
        return $this->rich_text_object;
    }
    private function clean_whitespace(): void
    {
        foreach ($this->rich_text_object->get_rich_text_elements() as $key => $element) {
    ...
    private function build_text_run(): void
    {
        $text = $this->string_data;
        if (trim($text) === '') {
            return;
        }
        $richtext_run = $this->rich_text_object->create_text_run($this->string_data);
        $font = $richtext_run->get_font();
        if ($font !== null) {
            if ($this->face) {
                $font->set_name($this->face);
            }
            if ($this->size) {
                $font->set_size($this->size);
            }
            if ($this->color) {
                $font->set_color(new Color('ff' . $this->color));
            }
            if ($this->bold) {
                $font->set_bold(true);
            }
            ...
            if ($this->underline) {
                $font->set_underline(Font::UNDERLINE_SINGLE);
            }
            if ($this->superscript) {
    ...
    private function rgb_to_colour(string $rgb_value): string
    {
        preg_match_all('/\d+/', $rgb_value, $values);
        foreach ($values[0] as &$value) {
            $value = str_pad(dechex((int) $value), 2, '0', STR_PAD_LEFT);
        }
        return implode('', $values[0]);
    }
    public static function colour_name_lookup(string $color_name): string
    {
        /** @var string[] */
        $temp = static::COLOUR_MAP;
        return $temp[$color_name] ?? '';
    }
    protected function start_font_tag(Dom_Element $tag): void
    {
        $attrs = $tag->attributes ?? [];
        /** @var DOMAttr $attribute */
        foreach ($attrs as $attribute) {
            $attribute_name = strtolower($attribute->name);
            $attribute_name = preg_replace('/^html:/', '', $attribute_name) ?? $attribute_name;
            // in case from Xml spreadsheet
            $attribute_value = $attribute->value;
            if ($attribute_name === 'color') {
                if (preg_match('/rgb\s*\(/', $attribute_value)) {
                    $this->{$attribute_name} = $this->rgb_to_colour($attribute_value);
                } elseif (str_starts_with(trim($attribute_value), '#')) {
                    $this->{$attribute_name} = ltrim($attribute_value, '#');
                } else {
                    $this->{$attribute_name} = static::colour_name_lookup($attribute_value);
                }
            } elseif ($attribute_name === 'face' || $attribute_name === 'size') {
                $this->{$attribute_name} = $attribute_value;
            }
        }
    }
    protected function end_font_tag(): void
    {
        $this->face = $this->size = $this->color = null;
    }
    protected function start_bold_tag(): void
    {
        $this->bold = true;
    }
    protected function end_bold_tag(): void
    {
        $this->bold = false;
    }
    protected function start_italic_tag(): void
    {
        $this->italic = true;
    }
    protected function end_italic_tag(): void
    {
        $this->italic = false;
    }
    protected function start_underline_tag(): void
    {
        $this->underline = true;
    }
    protected function end_underline_tag(): void
    {
        $this->underline = false;
    }
    protected function start_subscript_tag(): void
    {
        $this->subscript = true;
    }
    protected function end_subscript_tag(): void
    {
        $this->subscript = false;
    }
    protected function start_superscript_tag(): void
    {
        $this->superscript = true;
    }
    protected function end_superscript_tag(): void
    {
        $this->superscript = false;
    }
    protected function start_strikethru_tag(): void
    {
        $this->strikethrough = true;
    }
    protected function end_strikethru_tag(): void
    {
        $this->strikethrough = false;
    }
    public function break_tag(): void
    {
        $this->string_data .= "\n";
    }
    private function parse_text_node(Dom_Text $text_node): void
    {
        if ($this->preserve_white_space) {
            $dom_text = $text_node->node_value ?? '';
        } else {
            $dom_text = (string) preg_replace('/\s+/u', ' ', str_replace(["\r", "\n"], ' ', $text_node->node_value ?? ''));
        }
        $this->string_data .= $dom_text;
        $this->build_text_run();
    }
    public function add_start_tag_callback(string $tag, callable $callback): void
    {
        $this->start_tag_callbacks[$tag] = $callback;
    }
    public function add_end_tag_callback(string $tag, callable $callback): void
    {
        $this->end_tag_callbacks[$tag] = $callback;
    }
    /** @param callable[] $callbacks */
    private function handle_callback(Dom_Element $element, string $callback_tag, array $callbacks): void
    {
        if (isset($callbacks[$callback_tag])) {
            $element_handler = $callbacks[$callback_tag];
            call_user_func($element_handler, $element, $this);
        }
    }
    private function parse_element_node(Dom_Element $element): void
    {
        $callback_tag = strtolower($element->node_name);
        $this->stack[] = $callback_tag;
        $this->handle_callback($element, $callback_tag, $this->start_tag_callbacks);
        $this->parse_elements($element);
        array_pop($this->stack);
        $this->handle_callback($element, $callback_tag, $this->end_tag_callbacks);
    }
    private function parse_elements(Dom_Node $element): void
    {
        foreach ($element->child_nodes as $child) {
            if ($child instanceof Dom_Text) {
                $this->parse_text_node($child);
            } elseif ($child instanceof Dom_Element) {
                $this->parse_element_node($child);
            }
        }
    }
}