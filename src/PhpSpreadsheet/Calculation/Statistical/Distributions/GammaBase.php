...
abstract class Gamma_Base
{
    ...
    protected static function calculate_distribution(float $value, float $a, float $b, bool $cumulative): float
    {
        if ($cumulative) {
            return self::incomplete_gamma($a, $value / $b) / self::gamma_value($a);
        }
        return 1 / ($b ** $a * self::gamma_value($a)) * $value ** ($a - 1) * exp(-($value / $b));
    }
    /** @return float|string */
    protected static function calculate_inverse(float $probability, float $alpha, float $beta)
    {
        $x_lo = 0;
        $x_hi = $alpha * $beta * 5;
        $dx = 1024;
        ...
        while (abs($dx) > Functions::PRECISION && ++$i <= self::MAX_ITERATIONS) {
            ...
            $result = self::calculate_distribution($x, $alpha, $beta, true);
            ...
            $pdf = self::calculate_distribution($x, $alpha, $beta, false);
    ...
    //
    public static function incomplete_gamma(float $a, float $x): float
    {
        static $max = 32;
        $summer = 0;
        for ($n = 0; $n <= $max; ++$n) {
    ...
    //
    public static function gamma_value(float $value): float
    {
        if ($value == 0.0) {
            return 0;
        }
    ...
    private static float $log_gamma_cache_result = 0.0;
    private static float $log_gamma_cache_x = 0.0;
    /**
     * logGamma function.
     *
     ...
     */
    public static function log_gamma(float $x): float
    {
        if ($x == self::$log_gamma_cache_x) {
            return self::$log_gamma_cache_result;
        }
        $y = $x;
        if ($y > 0.0 && $y <= self::LOG_GAMMA_X_MAX_VALUE) {
            if ($y <= self::EPS) {
                ...
                $res = self::log_gamma1($y);
            } elseif ($y <= 4.0) {
                $res = self::log_gamma2($y);
            } elseif ($y <= 12.0) {
                $res = self::log_gamma3($y);
            } else {
                $res = self::log_gamma4($y);
            }
        } else {
            // --------------------------
        ...
        // ------------------------------
        self::$log_gamma_cache_x = $x;
        self::$log_gamma_cache_result = $res;
        return $res;
    }
    private static function log_gamma1(float $y): float
    {
        // ---------------------
        //    EPS .LT. X .LE. 1.5
        // ---------------------
    ...
    private static function log_gamma2(float $y): float
    {
        // ---------------------
        //    1.5 .LT. X .LE. 4.0
        // ---------------------
    ...
    protected static function log_gamma3(float $y): float
    {
        // ----------------------
        //    4.0 .LT. X .LE. 12.0
        // ----------------------
    ...
    protected static function log_gamma4(float $y): float
    {
        // ---------------------------------
