...
class MD5
{
    ...
    private int $c;
    private int $d;
    private static int $all_one_bits;
    /**
     * MD5 stream constructor.
     */
    public function __construct()
    {
        self::$all_one_bits = self::signed_int(0xffffffff);
        $this->reset();
    }
    /**
    ...
    {
        $this->a = 0x67452301;
        $this->b = self::signed_int(0xefcdab89);
        $this->c = self::signed_int(0x98badcfe);
        $this->d = 0x10325476;
    }
    /**
    ...
    {
        $unpacked = unpack('V16', $data) ?: throw new Reader_Exception('unable to unpack data');
        /** @var int[] */
        ...
        $this->a = $this->a + $A & self::$all_one_bits;
        $this->b = $this->b + $B & self::$all_one_bits;
        $this->c = $this->c + $C & self::$all_one_bits;
        $this->d = $this->d + $D & self::$all_one_bits;
    }
    ...
    private static function step(callable $func, int &$A, int $B, int $C, int $D, int $M, int $s, int $t): void
    {
        ...
        $A = (int) ($A + $temp + $M + $t) & self::$all_one_bits;
        $A = self::rotate($A, $s);
        $A = $B + $A & self::$all_one_bits;
    }
    /** @param float|int $result may be float on 32-bit system */
    private static function signed_int($result): int
    {
    ...
    private static function rotate(int $decimal, int $bits): int
    {
        ...
        return self::signed_int(bindec(substr($binary, $bits) . substr($binary, 0, $bits)));
    }
}