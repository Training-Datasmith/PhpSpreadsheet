...
class Bif_Fwriter
{
     ...
     */
    private static ?int $byte_order = null;
    /**
     ...
     */
    public static function get_byte_order(): int
    {
        if (!isset(self::$byte_order)) {
     ...
     */
    public function write_data(string $data): string
    {
        if (strlen($data) - 4 > $this->limit) {
            $data = $this->add_continue($data);
        }
     ...
     *                       0x0010 Worksheet
     */
    protected function store_bof(int $type): void
    {
        $record = 0x809;
     ...
     */
    protected function store_eof(): void
    {
        $record = 0xa;
        // Record identifier
        $length = 0x0;
     ...
     * Writes Excel EOF record to indicate the end of a BIFF stream.
     */
    public function write_eof(): string
    {
        $record = 0xa;
        // Record identifier
        $length = 0x0;
     ...
     */
    private function add_continue(string $data): string
    {
        $limit = $this->limit;
        $record = 0x3c;
        // Record identifier
